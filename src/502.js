...
const sorted = (projects) => projects.sort((a, b) => capital(a) - capital(b));
 ...
 * @return {number}
 */
export const findMaximizedCapital = (k, w, p, c) => {
  let totalCapital = w;
  let completed = 0;
  let pIndex = 0;
  const projects = sorted(zip(p, c));
  const heap = new MaxPriorityQueue({
    priority: profit,
  });

  while (completed++ < k) {
    while (pIndex < projects.length && capital(projects[pIndex]) <= totalCapital) {
      heap.enqueue(projects[pIndex++]);
    }
    if (heap.isEmpty()) {
      break;
    }
    totalCapital += heap.dequeue().priority;
  }
